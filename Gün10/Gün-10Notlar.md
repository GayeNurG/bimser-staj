...
Veri gönderilmeden önce TCP/IP modeli katmanlarına göre kapsüllenir. Uygulama katmanında oluşturulan veri, taşıma katmanında TCP başlığı, internet katmanında IP başlığı ve ağ erişim katmanında çerçeve bilgileri eklenerek fiziksel ortama gönderilir.
...
## 5. Ağ Cihazlarının Rolü

Paketler hedefe ulaşırken ağ cihazları farklı görevler üstlenir:

- **Switch:** Aynı yerel ağ içerisinde MAC adreslerini kullanarak iletim yapar.
- **Router:** IP adreslerine göre paketleri farklı ağlara yönlendirir.
- **Firewall:** Güvenlik kurallarına göre hangi trafiğin geçeceğine veya engelleneceğine karar verir.

## 6. Yanıt ve Kapsül Açma (Decapsulation)
