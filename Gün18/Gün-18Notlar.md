...
Proxy
Proxy, kullanıcıların internete (veya bir sunucuya) doğrudan değil, ara bir sunucu üzerinden erişmesini sağlıyor. İki türü var: forward proxy (iç ağdaki kullanıcıların dışarıya çıkışını bir noktadan geçirir) ve reverse proxy (dışarıdan gelen isteklerin iç sunuculara ulaşmadan önce geçtiği nokta). Amaç; trafiği tek bir noktadan kontrol etmek, filtrelemek ve kaydetmek.
...
Toplama (Collection): Farklı kaynaklardan (Linux auth.log, Windows Event Log, firewall, proxy vs.) logları çekip tek bir yere getirme.
Normalleştirme (Normalization): Her kaynağın kendi log formatı farklı olduğu için (biri syslog, biri Windows Event XML, biri CSV gibi), bunları ortak/standart bir yapıya çevirme. Böylece "kullanıcı adı" alanı her kaynakta farklı isimle geçse bile SIEM içinde tek bir alan olarak aranabilir hale gelir.
...
Uyarı Üretme (Alerting): Belirli koşullar sağlandığında (örneğin bir hesaba art arda 10 başarısız giriş) otomatik alarm üretme.
